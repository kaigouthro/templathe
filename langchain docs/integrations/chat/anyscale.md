...
os.environ["ANYSCALE\_API\_KEY"] = getpass()  
...
```python
from langchain.chat\_models import ChatAnyscale  
  
chats = {  
 model: ChatAnyscale(model\_name=model, temperature=1.0)  
 for model in ChatAnyscale.get\_available\_models()  
}  
...
```text
 dict\_keys(['meta-llama/Llama-2-70b-chat-hf', 'meta-llama/Llama-2-7b-chat-hf', 'meta-llama/Llama-2-13b-chat-hf'])  
...
async def get\_msgs():  
 tasks = [  
 chat.apredict\_messages(messages)  
...
```python
response\_dict = asyncio.run(get\_msgs())  
  
for model\_name, response in response\_dict.items():  
 print(f'\t{model\_name}')  
 print()  
