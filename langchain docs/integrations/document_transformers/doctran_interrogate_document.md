...
from langchain.document\_transformers import DoctranQATransformer  
...
qa\_transformer = DoctranQATransformer()  
transformed\_document = await qa\_transformer.atransform\_documents(documents)  
...
```python
transformed\_document = await qa\_transformer.atransform\_documents(documents)  
print(json.dumps(transformed\_document[0].metadata, indent=2))  
