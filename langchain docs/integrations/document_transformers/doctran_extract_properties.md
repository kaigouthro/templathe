...
from langchain.document\_transformers import DoctranPropertyExtractor  
...
properties = [  
 ...
 "type": "string",  
 "enum": ["update", "action\_item", "customer\_feedback", "announcement", "other"],  
 "required": True,  
 },  
 {  
 "name": "mentions",  
...
property\_extractor = DoctranPropertyExtractor(properties=properties)  
...
```python
extracted\_document = await property\_extractor.atransform\_documents(  
...
```python
print(json.dumps(extracted\_document[0].metadata, indent=2))  

```

```text
 {  
 "extracted\_properties": {  
 "category": "update",  
 "mentions": [  
 "John Doe",  
 "Jane Smith",  
