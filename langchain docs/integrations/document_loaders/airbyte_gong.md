...
```python
{  
 "access\_key": "<access key name>",  
 "access\_key\_secret": "<access key secret>",  
...
```python
from langchain.document\_loaders.airbyte import AirbyteGongLoader  
  
config = {  
 # your gong configuration  
}  
  
loader = AirbyteGongLoader(config=config, stream\_name="calls") # check the documentation linked above for a list of all streams  
...
loader = AirbyteGongLoader(config=config, record\_handler=handle\_record, stream\_name="calls")  
docs = loader.load()  
...
incremental\_loader = AirbyteGongLoader(config=config, stream\_name="calls", state=last\_state)  
